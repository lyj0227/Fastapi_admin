    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if code == 500:
            logger.add(
                sys.stderr,
                colorize=True,
                format=f"<red>{method} - {code}</red> - <bold><i>{path} - {http_type}</i></bold> - <blue>{current_time_china}</blue>",
                level="ERROR",
            )
            ...
            logger.add(
                sys.stderr,
                colorize=True,
                format=f"<green>{method}- {code}</green> - <bold><i>{path} - {http_type}</i></bold> - <blue>{current_time_china}</blue>",
                level="INFO",
            )
